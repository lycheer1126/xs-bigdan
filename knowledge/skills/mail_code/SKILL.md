...
- **注册后最高优先（2026-09 实战命中的接管形态）**：注册接口若**忽略密码校验**——
  任意非空密码(1-32 字符)都返回 success=true + 合法 JWT/token——立即升级：
  ① 检查 token 权限面（tenantId/有效期/可访问哪些业务 API，值池联动逐接口注入）；
  ② **换他人邮箱重试**（知道目标用户邮箱即可零凭据接管其账号）→ B2C 用户全量接管，高危起步。
- **邮箱验证码类漏洞**（次优先）：验证码可爆破（重复提交不同码）、返回包可篡改（响应里改校验字段）、
  注册接口可指定他人邮箱、激活链接可预测。
