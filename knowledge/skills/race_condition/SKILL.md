...
## 业务场景触发词（实战清单，功能点遍历时命中即测并发）

> 来源：实战碎片消化。这些场景的本质都是"限频/限额/互斥只在前端"。

| 场景 | 打法 | 期望 |
|---|---|---|
| 开发票/凭证 | 同一订单并发开发票 | 多张发票 |
| 购买+提现同发 | 两个包同时提交 | 余额负数/双花 |
| 先囤低价单再支付 | 多次生成低价订单，支付阶段统一替换 | 价差套利 |
| 限额类（私信N条/签到/抽奖次数） | 并发提交绕过计数 | 超限成功 |
| 新人礼/首单优惠 | 并发首次下单 | 多次享受 |
| 优惠券叠加 | 并发核销同一券 | 一券多用 |
