...
- CAM 凭证路径 404 **必须再打** `/cam/service-role-security-credentials/<角色名>`（腾讯云两套路径）
- 回环 403 ≠ 元数据被拦（只拦回环字符串、不拦元数据域名）——换元数据域名继续打
- 固定 POST 代理 405 → 用 302 redirect 让代理跟跳转（转成 GET）
- 公开 GOPROXY：模块路径写自己域，`go-import` 用 **hg**（git HTTPS 常超时）
