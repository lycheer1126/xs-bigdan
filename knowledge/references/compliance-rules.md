...
## 五、本地完全授权场景说明（本项目默认形态）

**本项目默认用于本地授权渗透测试（甲方书面授权），漏洞详情不公开：**

- 证据（evidence/）、Digest、最终报告均**保留完整原始数据，无需脱敏/打码**——真实请求包、真实响应、真实数据样本原样落盘。
- 操作边界（≤5条越权证明、禁批量导出、禁删改真实数据、禁DoS等）**仍然适用**——"不脱敏"不等于"可批量拉取"，拉取量的合规限制不变。
- 若某次任务需要对外提交（SRC 平台、众测平台），在 BRIEF.md 中显式声明"对外提交，需要脱敏"，agent 按声明对证据和报告做脱敏处理。
